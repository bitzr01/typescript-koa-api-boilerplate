...
describe('errorHandler', () => {
  ...
  it('Should handle the error when is a AppError but return 500 for invalid code', async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const ctx: any = {};
    const logger = pino({ name: 'test', level: 'silent' });
    const spy = sinon.spy(logger, 'error');
    ...
    await errorHandlerMiddleware(ctx, () => Promise.reject(new AppError(100000000, 'unknown error')));
    expect(spy.calledOnce).equals(true);
    expect(spy.args[0][1]).instanceof(AppError);
    expect(ctx.status).equals(500);
    expect(ctx.body).eql({
      code: 100000000,
